...
import { chatWithManus, getMessages } from '../api'

// 设置页面标题和元数据
useHead({
  title: 'YuManus 智能体 - SakuraYuManus 智能体应用平台',
  meta: [
    {
      name: 'description',
      content: 'YuManus 智能体是SakuraYuManus 智能体应用平台的全能助手，能解答各类专业问题，提供精准建议和解决方案'
    },
    {
      name: 'keywords',
      content: 'YuManus 智能体,智能助手,专业问答,AI问答,专业建议,Sakura,AI智能体'
    }
  ]
})
...
const uid = localStorage.getItem('userId')  // 持久化 userId，不受 Redis 影响
...
const sendMessage = (message) => {
  ...
  eventSource = chatWithManus(message, uid)
...
// 页面加载时加载历史对话
onMounted(async () => {
  const agentChatId = uid ? 'agent_chat_' + uid : null
  // 构造完整 conversationId，与后端保存时格式一致: userId:chatId
  const conversationId = uid && agentChatId ? uid + ':' + agentChatId : null
  if (conversationId) {
    try {
      const history = await getMessages(agentChatId, conversationId)
      if (history && history.length > 0) {
        messages.value = history.map(m => ({
          content: m.content, isUser: m.role === 'user', type: '', time: Date.now()
        }))
        return
      }
    } catch (e) { /* 首次使用或无历史 */ }
  }
  addMessage('你好，我是 YuManus 智能体。我可以解答各类问题，联网搜索最新信息，请问有什么可以帮助你的吗？', false)
})
